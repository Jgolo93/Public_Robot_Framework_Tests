*** Settings ***
Documentation     Dashboard Bubble Count Verification - RIPPLE FIBER ACCORDION
...               This test validates that bubble counts on the Ripple Fiber accordion
...
# Ripple Fiber Accordion (Accordion 2)
${ACCORDION_2}    //*[@id="ui-id-3"]

# List to track mismatches
@{MISMATCHES}

*** Test Cases ***
Verify Ripple Fiber Accordion Bubble Counts
    [Documentation]    Validates all bubble counts in the Ripple Fiber accordion
    Log into Portal
    Navigate to Service Dashboard
    Validate Ripple Fiber Accordion Bubbles
...
Validate Ripple Fiber Accordion Bubbles
    [Documentation]    Validates all bubbles in the Ripple Fiber accordion (Accordion 2)

    Log To Console    ${\n}========================================
    Log To Console    VALIDATING RIPPLE FIBER ACCORDION
    Log To Console    ========================================
    ...
    ${accordion_xpath}=    Set Variable    ${ACCORDION_2}
    ${accordion_name}=     Set Variable    Ripple Fiber
    ${panel_xpath}=        Set Variable    //*[@id="ui-id-4"]

    Log To Console    ${\n}Testing Accordion: "${accordion_name}"
    Log To Console    Expanding accordion...

    # Expand Ripple Fiber accordion
    Click Element    ${accordion_xpath}
    Sleep    2s

    # Find all bubbles in Ripple Fiber accordion
    ...
    FOR    ${bubble_idx}    IN RANGE    1    ${total_bubbles}+1
        ...
        Wait Until Page Contains Element    ${ACCORDION_2}   timeout=30s
...
Report Final Results
    ...
    Log To Console    ${\n}${\n}╔════════════════════════════════════════════════════════════╗
    Log To Console    ║        RIPPLE FIBER ACCORDION - FINAL REPORT               ║
    Log To Console    ╚════════════════════════════════════════════════════════════╝
