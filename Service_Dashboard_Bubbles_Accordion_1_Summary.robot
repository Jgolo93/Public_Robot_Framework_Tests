*** Settings ***
Documentation     Dashboard Bubble Count Verification - SUMMARY ACCORDION
...               This test validates that bubble counts on the Summary accordion
...
# Summary Accordion (Accordion 1)
${ACCORDION_1}    //*[@id="ui-id-1"]

# List to track mismatches
@{MISMATCHES}

*** Test Cases ***
Verify Summary Accordion Bubble Counts
    [Documentation]    Validates all bubble counts in the Summary accordion
    Log into Portal
    Navigate to Service Dashboard
    Validate Summary Accordion Bubbles
...
Validate Summary Accordion Bubbles
    [Documentation]    Validates all bubbles in the Summary accordion (Accordion 1)

    Log To Console    ${\n}========================================
    Log To Console    VALIDATING SUMMARY ACCORDION
    Log To Console    ========================================
    ...
    ${accordion_xpath}=    Set Variable    ${ACCORDION_1}
    ${accordion_name}=     Set Variable    Summary
    ${panel_xpath}=        Set Variable    //*[@id="ui-id-2"]

    Log To Console    ${\n}Testing Accordion: "${accordion_name}"
    Log To Console    Note: Summary accordion is already open by default

    Sleep    1s

    # Find all bubbles in Summary accordion
    ...
    FOR    ${bubble_idx}    IN RANGE    1    ${total_bubbles}+1
        ...
        Wait Until Page Contains Element    ${ACCORDION_1}   timeout=30s

        Log To Console    → Waiting for page to fully load...
        Wait Until Element Is Not Visible    css:.loading    timeout=50s
        Sleep    5s

        Log To Console    → Summary accordion remains open (default state)
...
Report Final Results
    ...
    Log To Console    ${\n}${\n}╔════════════════════════════════════════════════════════════╗
    Log To Console    ║          SUMMARY ACCORDION - FINAL REPORT                  ║
    Log To Console    ╚════════════════════════════════════════════════════════════╝
